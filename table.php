...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && !empty($_GET)) {
    if (isset($_GET['action']) && $_GET['action'] == 'delete') {
        $field = $_GET['field'];
        $sql = "ALTER TABLE `$students` DROP COLUMN `$field`;";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST)) {
    if (isset($_POST['field']) && isset($_POST['var']) && !empty($_POST['var'])) {
        ...
        $new = $_POST['var'];
        $type = $_GET['type'];
        $sql = "ALTER TABLE `$students` CHANGE `$old` `$new` $type";
        $statement = $pdo->prepare($sql);
        $statement->execute();
    ...
    if (isset($_POST['type']) && isset($_POST['types']) && !empty($_POST['types'])) {
        ...
        $field = $array[1];
        $type = $array[0];
        $sql = "ALTER TABLE `$students` CHANGE `$field` `$field` $type";
        $statement = $pdo->prepare($sql);
        $statement->execute();
    }
}
...
<table>
    ...
    <?php if (!empty($results)) { foreach ($results as $value) :?>
        <tr>
            <td><?=$value['Field']?></td>
            <td><?=$value['Type']?></td>
            <td><a class="done" href="<?="?table=$students&field=".$value['Field']."&type=".$value['Type']."&action=edit"?>">Изменить</a><a class="done" href="<?="?table=$students&field=".$value['Field']."&action=delete"?>">Удалить</a></td>
            <td>
